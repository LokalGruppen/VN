...
if ($item->data_type == 'tour') {
    ...
    $box3 .= '<span class="btn btn-success btn-block btn-lg mt-auto">' . JText::_('COM_VISITNORDIC_TOUR_VIEWOFFER') . '</span>';
...
<a href="<?php echo $link; ?>"<?php echo $this->options->get('main_content') ? ' class="list"' : ''; ?>>
    <div class="row py-3">
        <div class="col-12 col-md-6 <?php echo $this->options->get('main_content') ? 'col-lg-6' : 'col-lg-5'; ?>">
            <?php if ($image): ?>
        ...
        <div class="d-flex flex-column col-12 col-md-6 <?php echo $this->options->get('main_content') ? !$right ? 'col-lg-6' : 'col-lg-6' : 'col-lg-7 pl-lg-3 my-sm-2'; ?>">
            ...
            <?php if ($text): ?>
                <p style="font-size:12px;" class="<?php echo $this->options->get('main_content') ? 'mb-auto overflow-fade' : 'text-muted small m-0 d-none d-lg-block'; ?>"><?php echo $text; ?></p>
                <?php if ($info && $this->options->get('main_content')): ?>
                    <hr class="my-2">
                    <div style="font-size:12px;" class="text-muted align-bottom">
                        <?php echo $info; ?>
                    </div>
                <?php endif; ?>
            <?php endif; ?>
